...
export async function saveRecipientFromTransfer(transfer: Transfer) {
  ...
  const payload = {
    ...
    payout_provider_id: r.payoutProviderId ?? "AIRWALLEX_SANDBOX",
    beneficiary_details: r.payoutProviderId === "NIUM_SANDBOX" ? r.niumBeneficiaryFields ?? {} : r.airwallexBeneficiaryFields ?? {},
    beneficiary_transfer_method: r.payoutProviderId === "NIUM_SANDBOX" ? r.niumPayoutMethod ?? null : r.airwallexTransferMethod ?? null,
    beneficiary_schema_fetched_at: r.payoutProviderId === "NIUM_SANDBOX" ? r.niumSchemaFetchedAt ?? null : r.airwallexSchemaFetchedAt ?? null,
...
export async function loadSavedRecipients(): Promise<SavedRecipient[]> {
  ...
  return scopedRows.map((row: any) => ({
    ...
    payoutProviderId: row.payout_provider_id ?? "AIRWALLEX_SANDBOX",
    airwallexTransferMethod: row.payout_provider_id === "NIUM_SANDBOX" ? undefined : row.beneficiary_transfer_method,
    airwallexBeneficiaryFields: row.payout_provider_id === "NIUM_SANDBOX" ? undefined : row.beneficiary_details ?? {},
    airwallexSchemaFetchedAt: row.payout_provider_id === "NIUM_SANDBOX" ? undefined : row.beneficiary_schema_fetched_at,
    niumPayoutMethod: row.payout_provider_id === "NIUM_SANDBOX" ? row.beneficiary_transfer_method : undefined,
    niumBeneficiaryFields: row.payout_provider_id === "NIUM_SANDBOX" ? row.beneficiary_details ?? {} : undefined,
    niumSchemaFetchedAt: row.payout_provider_id === "NIUM_SANDBOX" ? row.beneficiary_schema_fetched_at : undefined,
