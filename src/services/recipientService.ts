...
function recipientFromTransferRow(row: any, userId: string): SavedRecipient {
  ...
  return {
    ...
    payoutProviderId: snapshot.payoutProviderId,
    recipientDetails: snapshot.recipientDetails,
    niumPayoutMethod: snapshot.niumPayoutMethod,
    niumBeneficiaryFields: snapshot.niumBeneficiaryFields,
    niumSchemaFetchedAt: snapshot.niumSchemaFetchedAt,
...
export async function saveRecipientFromTransfer(transfer: Transfer) {
  ...
  const payload = {
    ...
    payout_provider_id: null,
    beneficiary_details: r.recipientDetails ?? {},
    beneficiary_transfer_method: null,
    beneficiary_schema_fetched_at: null,
    provider_beneficiary_details: {
      airwallex: r.airwallexBeneficiaryFields ?? {},
      nium: r.niumBeneficiaryFields ?? {},
    },
    provider_schema_evidence: {
      airwallex: { transferMethod: r.airwallexTransferMethod ?? null, fetchedAt: r.airwallexSchemaFetchedAt ?? null },
      nium: { payoutMethod: r.niumPayoutMethod ?? null, fetchedAt: r.niumSchemaFetchedAt ?? null },
    },
...
export async function loadSavedRecipients(): Promise<SavedRecipient[]> {
  ...
  return scopedRows.map((row: any) => ({
    ...
    payoutProviderId: row.payout_provider_id ?? undefined,
    recipientDetails: row.provider_beneficiary_details ? row.beneficiary_details ?? {} : undefined,
    airwallexTransferMethod: row.provider_schema_evidence?.airwallex?.transferMethod ?? (row.payout_provider_id === "NIUM_SANDBOX" ? undefined : row.beneficiary_transfer_method),
    airwallexBeneficiaryFields: row.provider_beneficiary_details?.airwallex ?? (row.payout_provider_id === "NIUM_SANDBOX" ? undefined : row.beneficiary_details ?? {}),
    airwallexSchemaFetchedAt: row.provider_schema_evidence?.airwallex?.fetchedAt ?? (row.payout_provider_id === "NIUM_SANDBOX" ? undefined : row.beneficiary_schema_fetched_at),
    niumPayoutMethod: row.provider_schema_evidence?.nium?.payoutMethod ?? (row.payout_provider_id === "NIUM_SANDBOX" ? row.beneficiary_transfer_method : undefined),
    niumBeneficiaryFields: row.provider_beneficiary_details?.nium ?? (row.payout_provider_id === "NIUM_SANDBOX" ? row.beneficiary_details ?? {} : undefined),
    niumSchemaFetchedAt: row.provider_schema_evidence?.nium?.fetchedAt ?? (row.payout_provider_id === "NIUM_SANDBOX" ? row.beneficiary_schema_fetched_at : undefined),
