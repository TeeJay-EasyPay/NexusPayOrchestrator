# NexusPay Governance Index

## Governance Authority

The following documents define executive responsibilities,
authority, reporting standards, escalation procedures and
decision-making processes.

1. EXECUTIVE_CHARTER_TEMPLATE.md
2. CHIEF_ORCHESTRATOR_CHARTER.md
3. CHIEF_TECHNOLOGY_OFFICER_CHARTER.md
4. TESTING_DIRECTOR_CHARTER.md
5. EQAO_CHARTER.md
6. FOUNDER_COMMUNICATION_STANDARD.md
7. FOUNDER_BRIEFING_TEMPLATE.md

These documents are authoritative for governance.

Founder briefings created under [../founder-briefings](../founder-briefings) are required governance closure artefacts and must comply with the founder communication standard.

Executive reporting discovery is maintained in [../executive-reports/EXECUTIVE_REPORT_INDEX.md](../executive-reports/EXECUTIVE_REPORT_INDEX.md).

---

## Architecture Authority

The following documents define how NexusPay is designed
and must be considered during technical decision making.

- ARCHITECTURE_PRINCIPLES.md
- PROJECT_MAP.md
- UI_DESIGN_SYSTEM.md
- PROJECT_VISION.md

Architecture documents are authoritative for design and implementation decisions.

---

## Strategic Reference Documents

The following documents provide business and operational context.

- BUSINESS_EXECUTION_STRATEGY.md
- MISSION_CONTROL_REFINEMENT_REPORT.md

These documents provide guidance and context but do not override governance or architecture documents.

---

## Historical Reports

Implementation reports, audits, diagnostics and previous remediation reports are reference material only.

Historical reports do not override governance or architecture documents.

---

## Founder Reporting Authority

Founder communication and closure artefacts are governed through the founder briefing repository.

Primary navigation:
- [../founder-briefings/FOUNDER_BRIEFING_INDEX.md](../founder-briefings/FOUNDER_BRIEFING_INDEX.md)
- [../founder-briefings/PROGRAM_STATUS_LATEST.md](../founder-briefings/PROGRAM_STATUS_LATEST.md)
- [../founder-briefings/FOUNDER_ACTION_REGISTER.md](../founder-briefings/FOUNDER_ACTION_REGISTER.md)
- [../founder-briefings/briefings](../founder-briefings/briefings)
- [../founder-briefings/action-snapshots](../founder-briefings/action-snapshots)

---

## Executive Report Authority

Executive reporting discovery and navigation are maintained in:

- [../executive-reports/EXECUTIVE_REPORT_INDEX.md](../executive-reports/EXECUTIVE_REPORT_INDEX.md)

---

## Governance History

Governance implementation history and closure reports are maintained in:

- [../governance-history](../governance-history)

---

## Sprint Archive Authority

Institutional sprint history is maintained in:

- [../sprint-archives/SPRINT_ARCHIVE_INDEX.md](../sprint-archives/SPRINT_ARCHIVE_INDEX.md)
